...
describe('MidiTrack', () => {
  ...
  describe('建構函式', () => {
    it('應該正確初始化基本屬性', () => {
      ...
      expect(track.getName()).toBe('Test MIDI Track');
    ...
    it('應該正確初始化預設插件', () => {
      ...
      const trackWithPlugins = new MidiTrack(
        trackId,
        'Test Track',
        routing,
        [],
    ...
    it('應該在參數無效時拋出錯誤', () => {
      expect(() => new MidiTrack(null as any, 'Test Track', routing))
        .toThrow('Track ID cannot be null');
      expect(() => new MidiTrack(trackId, '', routing))
        .toThrow('Track name cannot be empty');
      expect(() => new MidiTrack(trackId, 'Test Track', null as any))
  ...
  describe('片段管理', () => {
    ...
    it('應該正確添加 MIDI 片段', () => {
      ...
      expect(track.getMidiClips()).toHaveLength(1);
      expect(track.getMidiClips()[0].equals(clipId1)).toBe(true);
    });
    ...
    it('應該正確移除 MIDI 片段', () => {
      ...
      expect(track.getMidiClips()).toHaveLength(1);
      expect(track.getMidiClips()[0].equals(clipId2)).toBe(true);
    });
    ...
    it('應該在添加非 ClipId 實例時拋出錯誤', () => {
      expect(() => track.addClip(null as any))
        .toThrow('Only MIDI clips can be added to MIDI tracks');
    });

    it('應該在移除非 ClipId 實例時拋出錯誤', () => {
      expect(() => track.removeClip(null as any))
        .toThrow('Only MIDI clips can be removed from MIDI tracks');
    });
  });
